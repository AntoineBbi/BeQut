...
#' @param tau skewness (vector) parameter. By default, this is a scalar between 0 and 1, but it can be a n-length vector spefifying the individual skewness.
...
#' @param object_lqmm Object returns by 'lqmm' or lslqmm function of BeQuT package to initialize parameter chains, default is \code{NULL}
...
#' lslqmm_median <- ALDreg(formFixed = log(serBilir) ~ year,
#'                         formRandom = ~ year,
#'                         formFixedScale = ~ year,
...
ALDreg <- function(formFixed,
  ...
  # add the consideration of object_lqmm and precision about beta distribution
  ...
  if(!is.null(tau) && prod(0<tau & 1>tau))
    stop("The 'tau' must be NULL or in ]0;1[.")
  ...
  if(length(tau)>1){
    if(length(tau)!= I)
      stop("Length of `tau` parameter must be egal to number of ID")
    }
